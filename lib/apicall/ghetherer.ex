defmodule Apicall.Gatherer do
  use GenServer
  @me Gatherer

  # api
  def start_link(worker_count) do
    GenServer.start_link(__MODULE__, worker_count, name: @me)
  end

  def done() do
    GenServer.cast(@me, :done)
  end

  def result(result) do
    GenServer.cast(@me, {:result, result})
  end

  # server
  def init(worker_count) do
    Process.send_after(self(), :kickoff, 0)
    {:ok, worker_count}
  end

  def handle_cast(:done, _worker_count = 1) do
    report_results()
    System.halt(0)
  end

  def handle_cast(:done, worker_count) do
    {:noreply, worker_count - 1}
  end

  def handle_cast({:result, result}, worker_count) do
    Apicall.Results.add(result)
    {:noreply, worker_count}
  end

  defp report_results() do
    IO.puts("Results:\n")

    Apicall.Results.result()
    |> Enum.each(&IO.inspect/1)
  end

  def handle_info(:kickoff, worker_count) do
    1..worker_count
    |> Enum.each(fn _ -> Apicall.WorkerSupervisor.add_worker() end)

    {:noreply, worker_count}
  end
end
